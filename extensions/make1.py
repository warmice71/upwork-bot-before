...
def addEducation(driver, education):
    ...
    global expFlag
    if expFlag:
		#driver.find_element(By.CLASS_NAME, 'd-block').find_element(By.TAG_NAME, 'button').click()
		#driver.find_element(By.XPATH, "//div[@class='d-block']/button[@class='air3-btn air3-btn-lg air3-btn-secondary-inverted air3-btn-circle']").click()
        driver.find_element(By.CLASS_NAME, 'd-block').find_element(By.TAG_NAME, 'button').click()
...
def configLast(driver, country, street, city, zipcode, phone, photo):
    ...
    sleep(1)
	#driver.find_element(By.CLASS_NAME, 'air3-input-prepend').find_element(By.TAG_NAME, 'input').send_keys(birthdate)
	#driver.find_element(By.CLASS_NAME, 'air3-input-prepend').find_element(By.TAG_NAME, 'input').send_keys(birthdate)
    waitInfinite(lambda: driver.find_element(By.CSS_SELECTOR, 'input[type="file"]').send_keys(os.path.join(os.getcwd(), 'users', photo)))
    # waitInfinite(lambda: driver.find_element(By.CSS_SELECTOR, 'input[aria-labelledby="date-of-birth-label"]').send_keys(birthdate))
